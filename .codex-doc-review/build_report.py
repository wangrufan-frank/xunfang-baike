...
def add_summary_table(document):
    rows = [
        ...
        (
            "安全治理",
            "开展敏感内容删除整改、访问范围控制、历史访问权限失效验证，并对公开法规和公开网页来源进行核验。",
            "降低不当信息展示风险，同时明确公开来源核验不替代单位内部业务、保密和发布审批。",
        ),
        (
            "推广反馈",
...
def build_report(output_path: Path) -> None:
    ...
    add_heading(document, "（二）把安全治理贯穿建设全过程", 2)
    add_body(
        document,
        "项目坚持最小范围使用和必要内容呈现，现阶段严格限定在大队内部试用。对可能涉及敏感警务信息、内部工作细节或者不适宜展示的内容，采取删除、脱敏、限制访问等措施；对历史访问权限进行失效验证，对页面更新前后的展示范围进行复核。内容建设不记录、不传播与使用目的无关的信息，不在报告和普通页面中展示安全配置细节。",
    )
