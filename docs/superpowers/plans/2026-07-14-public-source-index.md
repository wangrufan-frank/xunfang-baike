...
- `coverage_status: "verified"` 只表示知识点与网页之间存在已人工确认的主题、概念或局部内容相似，不表示来源完整证明整个知识点；部分相似或存在差异时在 `coverage_note` 中写明相似点和差异即可。
...
把每个法规知识点改为 `coverage_status: "verified"`，写入至少一个 `source_id` 和明确 `coverage_note`。若官方条文与页面表述不一致，但仍存在可说明的主题、概念或局部内容相似，可标记为 `verified`，同时写明相似点和差异；来源索引不承担判定页面法律结论正确的职责。
...
对数值参数、击打部位、开棍技术、上铐方法、喷射要领等确认公开网页正文存在相关主题、概念或局部相似内容。来源只支持一般介绍而不支持具体参数或动作时，仍可标记相似关系已核验，但必须在 `coverage_note` 中明确相似范围和未覆盖差异。
...
优先核验盘查流程、法言法语、武力升级程序和武器使用标准，回到现行法律法规和公安机关公开规范。发现页面把教学模型表述为法定程序时，只要公开网页存在相关程序或概念即可建立相似关联，并在 `coverage_note` 中记录差异。
...
不得使用与知识点完全无关的通用页面。主题相似、概念相似或局部内容相似均可建立关联，并由 `coverage_note` 说明；只有完全找不到可说明的相似公开内容时保持 `pending`。
