...
**Architecture:** 使用一个独立的 Python 构建脚本集中管理报告事实、正文段落和中文公文样式，通过 `python-docx` 生成 DOCX。使用结构测试检查标题、章节、数字口径、敏感词和页面设置，再通过 Windows 兼容的 LibreOffice→Poppler 流水线转为 PDF 和逐页 PNG，人工检查后按需迭代。

**Tech Stack:** Bundled Python 3、python-docx、OOXML、LibreOffice、Poppler、Codex documents renderer。

## Runtime Variables

以下 PowerShell 变量在执行本计划前统一定义一次，后续命令不再写入用户名、插件版本号或用户专属绝对路径：

```powershell
$BUNDLED_PYTHON = Join-Path $env:USERPROFILE '.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe'
$DOCUMENTS_CACHE = Join-Path $env:USERPROFILE '.codex\plugins\cache\openai-primary-runtime\documents'
$DOCS_VERSION_DIR = Get-ChildItem -LiteralPath $DOCUMENTS_CACHE -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
$DOCS_SKILL_DIR = Join-Path $DOCS_VERSION_DIR.FullName 'skills\documents'
$TABLE_GEOMETRY = Join-Path $DOCS_SKILL_DIR 'scripts\table_geometry.py'
...
$REPORT_DOCX = 'deliverables/关于巡防百科网站建设及推广应用情况的报告.docx'
$QA_DIR = Join-Path $env:TEMP 'xunfang-project-report-final-qa'
$LO_PROFILE = Join-Path $QA_DIR 'lo-profile'
```
...
**Interfaces:**
- Consumes: 最终 DOCX 路径。
- Produces: `& $BUNDLED_PYTHON .codex-doc-review/test_report.py -v` 可运行的结构验收测试。
...
class ReportTests(unittest.TestCase):
    ...
    def test_required_content(self):
        self.assertEqual(self.doc.paragraphs[0].text.replace("\n", ""), "关于“巡防百科”网站建设及推广应用情况的报告")
...
Expected: 当前完整测试套件全部 PASS；测试数量由测试运行器报告，不在计划中硬编码。
...
```powershell
& $BUNDLED_PYTHON $TABLE_GEOMETRY $REPORT_DOCX
```
...
**Files:**
- Create QA output: `$QA_DIR/page-*.png`
...
```powershell
New-Item -ItemType Directory -Path $QA_DIR, $LO_PROFILE -Force | Out-Null
$LO_PROFILE_URI = ([System.Uri]$LO_PROFILE).AbsoluteUri
& $SOFFICE "-env:UserInstallation=$LO_PROFILE_URI" --invisible --headless --norestore --convert-to pdf --outdir $QA_DIR $REPORT_DOCX
$REPORT_PDF = Join-Path $QA_DIR (([System.IO.Path]::GetFileNameWithoutExtension($REPORT_DOCX)) + '.pdf')
& $PDFTOPPM -png -r 171 $REPORT_PDF (Join-Path $QA_DIR 'page')
```
...
逐页以 100% 比例查看，确认：主标题按既定受控换行保持均衡两行、无挤压；正文无乱码、裁切或重叠；各级标题不孤悬在页尾；表格无跨页断裂、边界贴字或列宽失衡；页码居中且连续；署名和日期不单独落在空白页；全文字体替代可接受、无大面积空白。
...
```powershell
Get-Item -LiteralPath $REPORT_DOCX | Select-Object FullName,Length,LastWriteTime
```
...
再次运行完整 `unittest` 套件并确认全部 PASS；确认渲染页数与 PDF 页数一致；确认 DOCX 中没有未完成标记、内部路径、工具引用标记或未解释的技术命令。
