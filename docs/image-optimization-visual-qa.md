...
十二张正式截图均在配图滚入视口后以 `fullPage: false` 生成。为抵消浏览器截图时扣除的滚动条区域，桌面捕获视口校准为 1455×909，得到 6 张严格为 1440×900 的 PNG；移动捕获视口校准为 405×935，得到 6 张严格为 390×900 的 PNG。逐文件像素核验全部通过。
